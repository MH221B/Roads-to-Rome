...
const AdminPendingCourses: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const res = await getCoursesByStatus('pending', 1, 20);
        if (!active) return;
        setCourses(res.data || []);
      } catch (e) {
        if (!active) return;
    ...
    <Card>
      ...
      <CardContent>
        ...
        {loading ? (
          ...
          <div className="text-muted-foreground py-4 text-center">No pending courses found.</div>
        ) : (
